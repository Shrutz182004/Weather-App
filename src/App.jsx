...
import { HumidityIcon, SunriseIcon, VisibilityIcon,SunsetIcon } from './components/Icons';
...
<div className='flex items-center justify-center p-6 min-h-screen'>
        <div className='bg-transparent backdrop-filter backdrop-blur-md rounded-xl shadow-2xl p-8 max-w-md text-white w-full border border-white/30 relative z-10'>
          ...
          {!weather? (
       <form onSubmit={handleSearch} className='flex flex-col relative mb-6'>
  <input
    value={city}
  ...
  />
  {suggestion.length > 0 && (
    <div className='absolute top-12 left-0 right-0 bg-black/50 backdrop-blur-md shadow-md rounded z-10'>
      {suggestion.map((s) => (
        <button
          type='button'
          key={`${s.lat}-${s.lon}`}
            ...
            <div className=' mt-6 text-center transition-opacity duration-500'>
              ...
              <img src={`https://openweathermap.org/img/wn/${weather.weather[0].icon}@2x.png`} alt={ weather.weather[0].description} 
                ...
                {[
                  [HumidityIcon, "Humidity" , `${weather.main.humidity}%
                    (${getHumidityValue(weather.main.humidity)})`],
                     ...
                     [HumidityIcon, "Wind" , `${weather.wind.speed}m/s ${weather.wind.deg ?
               ...
               <div className='mt-6 text-sm'>
                <p><strong>FeelsLike:</strong>{convertTemperature(weather.main.feels_like, unit)} &deg;{unit}</p>
